...
# required and whnameich we can discard.
def should_keep_microsoft_symbol(symbol, calling_convention_decoration, namespaces):
    has_any_namespace = False
    if not namespaces:
        has_any_namespace = True
    else:
        for namespace in namespaces:
            if namespace in symbol:
                has_any_namespace = True
                break
    ...
    # clang::Type::getAs, as some of them are explicit specializations that are
...
# discarded.
def should_keep_itanium_symbol(symbol, calling_convention_decoration, namespaces):
    has_any_namespace = False
    if not namespaces:
        has_any_namespace = True
    else:
...
def extract_symbols(arg):
    get_symbols, should_keep_symbol, calling_convention_decoration, namespaces, lib = arg
    symbols = dict()
    for symbol in get_symbols(lib):
        symbol = should_keep_symbol(symbol, calling_convention_decoration, namespaces)
        if symbol:
...
if __name__ == '__main__':
    ...
    libs = list()
    inlibs = glob.glob(args.libdir + '/**/*.lib', recursive=True)
    for lib in inlibs:
    ...
    try:
        ...
        vals = [(get_symbols, should_keep_symbol, calling_convention_decoration, args.namespaces, x) for x in libs]
